...
CREATE TABLE IF NOT EXISTS `EstadisticasCOVID`.`INS` (
  ...
  `fis` VARCHAR(50) NULL,
  `fecha_de_muerto` DATE NULL,
  `fecha_diagnostico` DATE NULL,
  `fecha_recuperado` DATE NULL,
  `fecha_reporte_web` DATE NULL,
  PRIMARY KEY (`id_de_caso`),
  INDEX `INSINX` (`departamento` ASC, `atencionn` ASC, `fecha_diagnostico` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `EstadisticasCOVID`.`Bogota` (
  `fecha_diagnostico` INT NOT NULL,
  ...
  `estado` VARCHAR(50) NULL,
  PRIMARY KEY (`fecha_diagnostico`),
  INDEX `BOGINX` (`localidad` ASC, `estado` ASC) VISIBLE)
ENGINE = InnoDB;
